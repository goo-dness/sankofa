...
### 2026-07-20 — Layer architecture clarified, SL4 naming resolved

**Decided:** Sankofa's four-layer architecture is now formally documented in §1:
- Layer 1: Knowledge Foundation (complete)
- Layer 2: Computational Symbolic Engine (in progress) — code in `computation/`
- Layer 3: AI Layer / Litsi (next)
- Layer 4: Ùmà Layer (final)

"SL4" is the internal codename for the `computation/` directory, not a separate product name. The engine itself is Layer 2 — the Computational Symbolic Engine. All references to "SL4 query engine" or "SL4" in CONTEXT.md updated to use correct terminology.

**Coverage registry:** the `ingestion_coverage` table (model in `models/coverage.py`, migration `002_add_coverage`) is already built and populated (23 rows). The earlier documentation incorrectly described this as a "future dependency" — it was completed July 2026, not deferred.

**Why:** Naming confusion was creating ambiguity between the directory name (`computation/`) and what the code actually does (Layer 2: Computational Symbolic Engine). Clear layer numbering prevents this.

**Rules out:** Treating "SL4" as a product name or separate entity from Layer 2.
