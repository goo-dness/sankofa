...
from computation.epistemic import (
    EpistemicState,
    resolve_chain_epistemic_state_backward,
    resolve_chain_epistemic_state_forward,
...
def fetch_citations(db: Session, relationship_ids: list[int]) -> dict[int, list[dict]]:
    if not relationship_ids:
        return {}
    rows = (
        db.execute(CITATIONS_QUERY, {"relationship_ids": relationship_ids})
        .mappings()
        .all()
    )
...
def execute_single_hop(db: Session, source_name: str, relationship_type: str) -> dict:
    ...
    return {
        "epistemic_state": resolve_epistemic_state(
            db, results, source_name, relationship_type
        ),
...
def execute_two_hop_forward(
    ...
    return {
        "epistemic_state": resolve_chain_epistemic_state_forward(
            db, results, source_name, first_relationship, second_relationship
        ),
        "query_results": results,
        "citations": citations,
        "contradictions": detect_contradictions(results),
        "chain_weight": compute_chain_weights(results, is_neighborhood=False)
        if results
        else None,
    }


def execute_two_hop_backward(
    db: Session,
    target_name: str,
    first_relationship: str,
    second_relationship: str,
    ...
    return {
        "epistemic_state": resolve_chain_epistemic_state_backward(
            db, results, target_name, first_relationship, second_relationship
        ),
        "query_results": results,
        "citations": citations,
        "contradictions": detect_contradictions(results),
...
def execute_neighborhood(
    db: Session, entity_id: int, depth: int = 1, row_limit: int = 1000
) -> dict:
    rows = (
        db.execute(
            NEIGHBORHOOD_QUERY,
    ...
    results = [dict(r) for r in rows[:row_limit]]
    ...
    return {
        ...
        "citations": citations,
        "contradictions": detect_contradictions(results),
        "chain_weight": compute_chain_weights(results, is_neighborhood=True)
        if results
        else None,
        "truncated": truncated,
    }
